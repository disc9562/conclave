...
export const zh: Record<TranslationKey, string> = {
  ...
  'settings.terminal.description': '直接运行宿主机命令，用于安装插件、Skills、MCP 等扩展。桌面端已内置 DreamCoder 运行时；文档里的 claude <参数> 可替换成本地 claude 运行时，例如 claude plugin install ... 或 claude mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': '只会清理：dreamcoder-open-tabs、dreamcoder-session-runtime、dreamcoder-theme、dreamcoder-locale 和 dreamcoder.persistence.schemaVersion。',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/dreamcoder/settings.json — 直接编辑，保存时写入。',
  ...
  'settings.computerUse.modeLabel': '感知模式',
  'settings.computerUse.modeDescription': 'AI 如何"看到"你的屏幕。两种模式使用不同的技术原理，适用于不同类型的模型。',
  'settings.computerUse.modeVisionDesc': '截取屏幕截图，由多模态模型"看图"理解界面。点击坐标基于截图位置。',
  'settings.computerUse.modeVisionRequirement': '需要多模态模型（如 GPT-4o、Claude Sonnet）',
  'settings.computerUse.modeUiaDesc': '读取 Windows 无障碍树，将界面元素序列化为文本。模型按元素 ID 操作，无需"看图"。',
  'settings.computerUse.modeUiaRequirement': '纯文本模型即可（如 DeepSeek、GPT-4o-mini）',
  ...
  'settings.general.storagePortableDirPlaceholder': '选择一个用于保存 dreamcoder 数据的文件夹',
  ...
  'settings.general.storageMoveHint': '切换目录不会自动搬迁旧数据。如果希望旧会话继续出现，请把原目录下的 projects、skills、plugins、dreamcoder 等数据复制到新目录。要做便携包，建议把目录放在应用旁边并一起压缩。',
  ...
  'settings.general.notificationsTestTitle': 'DreamCoder 通知已启用',
