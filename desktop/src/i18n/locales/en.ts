export const en = {
  ...
  'settings.terminal.description': 'Run host-machine commands for plugin, skill, and MCP setup. The desktop app includes the DreamCoder runtime; replace documented claude <args> with the local claude runtime for example: claude plugin install ... or claude mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': 'Clears only: dreamcoder-open-tabs, dreamcoder-session-runtime, dreamcoder-theme, dreamcoder-locale, and dreamcoder.persistence.schemaVersion.',
  ...
  'settings.providers.settingsJsonDesc': '~/.claude/dreamcoder/settings.json — edit directly, will be written on save.',
  ...
  'settings.computerUse.modeLabel': 'Perception Mode',
  'settings.computerUse.modeDescription': 'How the AI "sees" your screen. The two modes use different technical approaches and work with different types of models.',
  'settings.computerUse.modeVisionDesc': 'Captures a screenshot and lets a multimodal model "look at" the UI. Click coordinates are based on the screenshot.',
  'settings.computerUse.modeVisionRequirement': 'Requires a multimodal model (e.g. GPT-4o, Claude Sonnet)',
  'settings.computerUse.modeUiaDesc': 'Reads the Windows accessibility tree and serializes UI elements as text. The model operates by element ID, no "vision" needed.',
  'settings.computerUse.modeUiaRequirement': 'Works with text-only models (e.g. DeepSeek, GPT-4o-mini)',
  ...
  'settings.general.storagePortableDirPlaceholder': 'Choose a folder for dreamcoder data',
  ...
  'settings.general.storageMoveHint': 'Switching directories does not migrate existing data. Copy projects, skills, plugins, dreamcoder, and related folders from the old directory if you want old sessions to keep appearing. For a portable bundle, keep this folder beside the app and zip them together.',
  ...
  'settings.general.notificationsTestTitle': 'DreamCoder notifications are enabled',
